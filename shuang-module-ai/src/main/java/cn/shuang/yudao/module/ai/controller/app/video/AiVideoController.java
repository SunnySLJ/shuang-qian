...
public class AiVideoController {
    ...
    public CommonResult<PageResult<AiImageRespVO>> getVideoPage(
        ...
        return success(PageResult.convert(pageResult, vo -> {
            AiImageRespVO vo = new AiImageRespVO();
            vo.setId(vo.getId());
            vo.setPrompt(vo.getPrompt());
            vo.setStatus(vo.getStatus());
            vo.setPicUrl(vo.getPicUrl());
            vo.setOutputUrl(vo.getOutputUrl());
            vo.setGenerationType(vo.getGenerationType());
            return vo;
        }));
    }
