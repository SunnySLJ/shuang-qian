...
const BASE_URL = import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:48080/app-api'
const TIMEOUT = 30000
...
async function request<T = unknown>(
  ...
  try {
    ...
    if (err instanceof Error) {
      ...
      if (err.message.includes('fetch') || err.message.includes('NetworkError') || err.message.includes('network') || err.message.includes('Failed to fetch')) {
        throw new Error('网络连接失败，请检查后端服务是否启动 (http://127.0.0.1:48080)')
      }
      throw new Error(err.message || '网络连接失败，请检查网络')
    }
