...
export interface CommissionRecord {
  id: number
  type: 'commission' | 'transfer' | 'recharge' | 'consume'
  title: string
...
export function getMyAgency() {
  return get<MyAgencyVO>('/agency/user/my')
}

/** 绑定上级代理 */
export function bindAgency(data: BindAgencyReqVO) {
  return post<Boolean>('/agency/user/bind', data)
}

/** 分配积分给用户 */
export function transferPoints(data: TransferPointsReqVO) {
  return post<Boolean>('/agency/point/transfer', data)
}

/** 获取我的下级列表 */
export function getChildren() {
  return get<ChildAgencyVO[]>('/agency/user/children')
}

/** 获取我的代理积分钱包 */
export function getAgencyWallet() {
  return get<AgencyWalletVO>('/agency/point/wallet')
}

/** 获取分佣记录（兼容前端已有结构） */
export function getCommissionRecords(params?: { pageNo?: number; pageSize?: number }) {
  return get<CommissionRecord[]>('/agency/commission/records', params)
}
