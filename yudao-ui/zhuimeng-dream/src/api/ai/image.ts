...
export function generateImage(data: ImageGenerateReqVO) {
  return post<number>('/app/ai/image/generate', data)
}

/** 获取我的图片列表（简单列表） */
export function getMyImageList(limit: number = 20) {
  return get<ImageRecord[]>('/app/ai/image/list', { limit })
}

/** 分页获取我的图片列表 */
export function getMyImagePage(params: { pageNo?: number; pageSize?: number }) {
  return get<ImagePageRespVO>('/app/ai/image/page', params)
}

/** 获取图片详情 */
export function getImageDetail(id: number) {
  return get<ImageRecord>('/app/ai/image/get', { id })
}

/** 删除图片 */
export function deleteImage(id: number) {
  return request.delete('/app/ai/image/delete', { params: { id } })
}
