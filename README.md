...
```sql
CREATE DATABASE shuang_pro DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```
...
```bash
# 1. 基础系统表
mysql -u root -p shuang_pro < sql/mysql/ruoyi-vue-pro.sql

# 2. 代理系统表
mysql -u root -p shuang_pro < sql/mysql/shuang-pro-agency.sql
```

### 配置修改

修改 `shuang-server/src/main/resources/application-dev.yml`:

```yaml
spring:
  datasource:
    url: jdbc:mysql://localhost:3306/shuang_pro?useSSL=false&serverTimezone=Asia/Shanghai
    username: root
...
访问 Swagger 文档：http://localhost:48080/swagger-ui.html
