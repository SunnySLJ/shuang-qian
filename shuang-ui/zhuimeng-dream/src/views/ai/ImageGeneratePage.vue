  ...
  <div class="image-generate-page">
    <div class="page-layout">
      ...
      <div class="input-panel glass-card">
        ...
        <div class="style-section">
          <div class="section-label">生成模型</div>
          <select v-model.number="selectedModelId" class="prompt-input">
        ...
        <div class="prompt-panel glass-card">
          <div class="prompt-panel-head">
            <div class="section-label" style="margin-bottom: 0;">Prompt 面板</div>
            <button class="btn-ghost" @click="showPromptPanel = !showPromptPanel">
              {{ showPromptPanel ? '收起' : '展开' }}
            </button>
          </div>
          <div v-if="showPromptPanel" class="prompt-panel-body">
            <div class="prompt-block">
              <div class="prompt-block-title">原始输入</div>
              <pre class="prompt-pre">{{ prompt || '暂无' }}</pre>
            </div>
            <div class="prompt-block">
              <div class="prompt-block-title">提交给模型的 Prompt</div>
              <pre class="prompt-pre">{{ composedPrompt }}</pre>
            </div>
            <div class="prompt-panel-actions">
              <button class="btn-ghost" @click="copyPrompt(composedPrompt)">复制成品 Prompt</button>
            </div>
          </div>
        </div>
...
import { generateImage, getImageDetail, type ImageRecord } from '@/api/ai/image'
import { getAiModelSimpleList, type AiModelOption } from '@/api/ai/model'
import { toast } from '@/utils/toast'
import { getLastImageModelId, setLastImageModelId } from '@/utils/aiModelPrefs'
...
async function generate() {
  ...
  try {
    const [width, height] = selectedSize.value.split(':').map(Number)
    const scale = 1024 / Math.max(width, height)
    const recordId = await generateImage({
      prompt: prompt.value,
      style: selectedStyle.value,
      width: Math.round(width * scale),
      height: Math.round(height * scale),
      modelId: selectedModelId.value,
      clientId: `image-generate-${Date.now()}`,
    })
    const detail = await pollImageResult(recordId)
    results.value.unshift({
      id: detail.id,
      url: detail.outputUrl || '',
...
async function pollImageResult(id: number): Promise<ImageRecord> {
  const maxAttempts = 40
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    const detail = await getImageDetail(id)
    if (detail.status === 20 && detail.outputUrl) return detail
    if (detail.status === 30) throw new Error(detail.errorMessage || '图片生成失败')
...
async function loadModels() {
  try {
    modelOptions.value = await getAiModelSimpleList(2)
    const lastModelId = getLastImageModelId()
    selectedModelId.value = modelOptions.value.some(model => model.id === lastModelId)
      ? lastModelId
...
.prompt-panel { padding: 16px; display: flex; flex-direction: column; gap: 12px; }
.prompt-panel-head { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
.prompt-panel-body { display: flex; flex-direction: column; gap: 12px; }
.prompt-block { display: flex; flex-direction: column; gap: 6px; }
.prompt-block-title { font-size: 12px; font-weight: 700; color: var(--color-text-secondary); }
.prompt-pre { margin: 0; white-space: pre-wrap; word-break: break-word; max-height: 180px; overflow: auto; padding: 12px; border-radius: 10px; background: rgba(255,255,255,0.04); color: var(--color-text-secondary); font-size: 12px; line-height: 1.6; }
